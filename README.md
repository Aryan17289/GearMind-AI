...
GearMind is a comprehensive predictive maintenance dashboard that monitors the health of industrial gears using machine learning. It supports **four gear types (Helical, Spur, Bevel, Worm)** with dedicated models, provides explainable AI insights, generates professional PDF reports, and includes an AI-powered copilot assistant.
...
- **Worm Gear:** Logistic Regression classifier (6 features)
...
- Available for all 4 gear types
...
```
gear_api.py
├── Multi-model loading (Helical, Spur, Bevel, Worm)
├── 16 API endpoints
...
├── worm_classifier.pkl                # Worm Logistic Regression
├── *_rul_regressor.pkl                # RUL models (all 4)
├── *_scaler.pkl                       # Scalers (all 4)
└── *_label_encoder.pkl                # Label encoders (all 4)

xai/
├── shap_artifacts.pkl                 # Helical SHAP
├── spur_shap_artifacts.pkl            # Spur SHAP
├── bevel_shap_artifacts.pkl           # Bevel SHAP
└── worm_shap_artifacts.pkl            # Worm SHAP
...
└── worm_gear_dataset.csv              # Worm training data
```
...
```bash
git clone https://github.com/Aryan17289/GearMind-AI.git
cd GearMind-AI
```
...
- Dynamic per-gear model performance table
- Metrics comparison chart
- Overall cross-gear comparison view
...
- **Framework:** FastAPI
- **ML Models:** XGBoost, SVM, Logistic Regression (scikit-learn)
- **XAI:** SHAP, LIME
- **Database:** SQLite
...
- **F1 Score:** 0.928
- **AUC:** 0.987
- **Features:** 6 (RPM, Input Torque, Oil Temperature, Output Speed, Vibration, Efficiency)
...
- [ ] Start backend and verify all 4 models load
- [ ] Start frontend and access dashboard
- [ ] Switch between gear types (Helical/Spur/Bevel/Worm)
...
- [ ] Check Model Comparison table with gear type dropdown
